    ...
    std::string& output)
{
    ...
    double ifd = 
        //actions::estimateInterfocalDistancePoints(potential, traj);
        actfinder.interfocalDistance(toPosVelCyl(initial_conditions));

    //std::ofstream strm;
    //if(ifd==1234)
    //    strm.open(("orb"+utils::pp(initial_conditions.x,6)+'_'+utils::pp(initial_conditions.vx,6)).c_str());
    ...
    for(size_t i=0; i<traj.size(); i++) {
        ...
        strm << utils::pp(actsF.Jr, 8)+' '+utils::pp(actsI.Jr, 8)+' '+utils::pp(actsF.Jz, 8)+' '+utils::pp(actsI.Jz, 8)+'\n';*/
        actF.add(actions::actionsAxisymFudge(potential, toPosVelCyl(traj[i]), ifd));
        actI.add(actfinder.actions(toPosVelCyl(traj[i])));
    }
...
#pragma omp parallel for schedule(dynamic,1)
    ...
    if(utils::verbosityLevel >= utils::VL_VERBOSE) {
        ...
        output << "E\tLcirc\tJphi\tJr\tJr_err\tJz\tJz_err\tJr,int\tJr_err,int\tJz,int\tJz_err,int\n";
