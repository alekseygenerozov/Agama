...
int main()
{
    ...
    std::vector<galaxymodel::Component> components;
    components.push_back(galaxymodel::Component(NULL, 1e-3, 40,  50, 4, guessForDensityInner));
    components.push_back(galaxymodel::Component(&dfOuter, 5e-3, 500, 50, 4));

    galaxymodel::SelfConsistentModel* model = 
        new galaxymodel::SelfConsistentModel(components, &progressReporter);
    // do a few iterations
    ...
    const potential::BaseDensity* guessForDensityOuter = model->getComponentDensity(1)->clone();
    delete model;  // destroy all traces
    ...
    components.clear();
    components.push_back(galaxymodel::Component(&dfInner, 1e-3, 40,  50, 6, guessForDensityInner));
    components.push_back(galaxymodel::Component(&dfOuter, 5e-3, 500, 50, 4, guessForDensityOuter));
    delete guessForDensityInner;
    delete guessForDensityOuter;  // not needed anymore

    model = new galaxymodel::SelfConsistentModel(components, &progressReporter, 1e-3, 1e5);
    ...
    guessForDensityOuter = model->getComponentDensity(1)->clone();
