...
%%%%%%%%%%%%%%
\subsubsection{Basis splines}  \label{sec:MathSplineDetails}

We often need to represent approximately an arbitrary function of one variable, defined on a finite or infinite interval.
This is conventionally achieved by using a basis-set expansion, that is, the function $f(x)$ is approximated as a weighted sum $\tilde f^{(M)}$ of a finite number ($M$) of basis functions, and the accuracy of approximation improves as we increase the number of terms:
\begin{subequations}
\begin{align}
...
\begin{align}
C_j \equiv \int \tilde f^{(M)}(x)\, B_j(x)\, dx = \sum_{i=1}^M A_i\, \mathsf{B}_{ij} \;,\quad
\mathsf{B}_{ij} \equiv \int B_i(x)\,B_j(x)\,dx
\end{align}
\end{subequations}
Now if we compute $C_j$ by putting the original function $f(x)$ into the above integral, and solve the resulting linear system to obtain $A_i$, this gives an approximation of the original function.

There are many possible choices of basis sets: for instance, for a periodic function the natural choice are the trigonometric functions, forming the Fourier expansion.
B-splines of degree $N$ are a set of piecewise-polynomial functions defined by the nodes of a grid: $x_i, i=1..M$. The number of basis functions is $M+N-1$, and each one is 
